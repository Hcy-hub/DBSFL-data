...
def cifar_noniid(args, dataset, num_users, num_items):
    ...
    cifar_ch_list = list(itertools.combinations(range(len(classes)), num_digit_noniid))
...
def fashion_mnist_iid(args,dataset, num_users, num_items):    
    dict_users, all_idxs = {}, [i for i in range(len(dataset))]     
    if len(dataset) == 60000:
        num_group = int(num_users/len(args.ratio_train))
        if args.strict_iid == True:
...
def fashion_mnist_noniid(args, dataset, num_users, num_items):
    """
    Sample non-I.I.D client data from MNIST dataset
    :param dataset:
    :param num_users:
...
def adult_iid(args,dataset, num_users, num_items):
    """
    ...
    if len(dataset) == 60000:
        ...
        c = set(np.random.choice(all_idxs, num_items, replace=True))
        for i in range(num_users):  
            dict_users[i] = copy.deepcopy(c)
            # print("\nDivide", len(all_idxs))                      
    return dict_users

def adult_noniid(args, dataset, num_users, num_items):
    """
